...
///
/// This is represented as number of nanoseconds, which given its 64 bit
/// width, means that a socket has a maximum lifetime of around 585 years.
///
/// This is an absolute timestamp within the "Socket Epoch".
#[derive(Copy, Clone, Debug, PartialEq, Eq, PartialOrd, Ord)]
pub struct SocketTime(u64);
...
impl SocketTime {
    ...
    pub const fn from_nanos(ns: u64) -> SocketTime {
        SocketTime(ns)
    }
    ...
    /// Returns the duration elapsed since the socket's creation.
...
impl Add<Duration> for SocketTime {
    ...
    fn add(self, rhs: Duration) -> SocketTime {
        SocketTime::from_nanos(self.0.saturating_add(rhs.as_nanos() as u64))
    }
}
...
impl Sub<Duration> for SocketTime {
    ...
    fn sub(self, rhs: Duration) -> SocketTime {
        SocketTime::from_nanos(self.0.saturating_sub(rhs.as_nanos() as u64))
    }
}
...
impl From<Duration> for SocketTime {
    fn from(value: Duration) -> Self {
        SocketTime::from_nanos(value.as_nanos() as u64)
    }
}
