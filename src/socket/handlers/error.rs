...
pub(crate) fn handle_abort(state: &mut State, ctx: &mut Context<'_, '_>, chunk: AbortChunk) {
...
pub(crate) fn handle_error(state: &mut State, ctx: &mut Context<'_, '_>, chunk: ErrorChunk) {
    if state.tcb().is_none() {
        return;
    }
    let message =
...
pub(crate) fn handle_unrecognized_chunk(state: &mut State, ctx: &mut Context<'_, '_>, chunk: UnknownChunk) -> bool {
